...
# Update these Supabase credentials
SUPABASE_URL = "https://kfxkbzuwtxokeahpezaz.supabase.co"
SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImtmeGtienV3dHhva2VhaHBlemF6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDExOTk2NTMsImV4cCI6MjA1Njc3NTY1M30.izhAN52nOzkR_sQiNzYPG-GmhfXad8ocE1Kq811GdEU"
...
def extract_content(video_path, paths, candidate_name, candidate_id):
    try:
        ...
        # Generate unique candidate_id for database
        db_candidate_id = f"CAND_{candidate_id}"
        ...
        evaluation_text, score = generate_ai_report(full_analysis, paths, api_key, db_candidate_id)
        ...
        try:
            ...
            result = supabase.table('emotion_detection').insert(emotion_data).execute()
            print("✓ Uploaded to emotion_detection")
            ...
            supabase.table('candidate_evaluation').insert(eval_data).execute()
            print("✓ Uploaded to candidate_evaluation")
...
def generate_ai_report(analysis_data, paths, api_key, candidate_id):
...
def get_candidate_evaluation(candidate_id):
    """Retrieve complete candidate evaluation from Supabase"""
    try:
        result = supabase.table('candidate_evaluation')\
            .select('*')\
